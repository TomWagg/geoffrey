...
for orcid, last_name in zip(orcids, last_names):
    papers_dict_list = ads_query.get_ads_papers(query=f"orcid:{orcid}", past_week=False,
                                                remove_known_papers=True)

    recent_papers = [paper for paper in papers_dict_list if paper["date"] > datetime.date(2023, 8, 1)]

    print(f"Found {len(recent_papers)} papers for {last_name}")

    ads_query.save_papers(recent_papers)
